...
public class ToyControler {
	...
	public List<Toy> get_Toy_batch(@PathVariable int pno, @PathVariable int psize){
		return ts.fetch_by_batch(pno,psize);
	}
	
	@GetMapping("/manufacturer/{name}")
	public List<Toy> get_Toy_by_manufacturer(@PathVariable String name){
		return ts.fetchByMname(name);
	}
	...
	public ResponseEntity<String> get_Toy(@PathVariable int id) {
		...
		if(t != null) {
			return ResponseEntity.status(HttpStatus.ACCEPTED).body(t.displayToy());
		}else {
